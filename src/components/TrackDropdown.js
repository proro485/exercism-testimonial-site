  ...
  useEffect(() => {
    async function fetchData() {
      ...
      for (let i = 0; i < tracks.length; i++) {
        tracksList[tracks[i].slug] = {
          title: tracks[i].title,
          icon: tracks[i].icon_url,
        };
      }
  ...
  return (
    ...
    >
      <button className="flex items-center">
        <img
          className="ml-2 mr-2 sm:mr-3 h-12"
          src={logo === 'all' ? hexLogo : `${logo}`}
          alt=""
        />
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="
                      ...
                      rounded-full"
                    checked={props.tracks[track] === props.whichTrack}
                    onChange={() => {}}
                  />
                  <img
                    className="mr-4 h-10"
                    src={
                      idx !== 0
                        ? `${allTracks[props.tracks[track]].icon}`
                        : hexLogo
                    }
                    alt=""
                  />
